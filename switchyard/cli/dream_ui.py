...
function selector(){let routes=data.selector||[];if(!routes.length)return empty('#selector','No route attribution yet. New server records include route_id; historical snapshots cannot be attributed.');let route=routes[0],items=route.targets.slice(0,12),rh=34,s=svgBox(12+items.length*rh),max=Math.max(...items.map(x=>x.calls),1);items.forEach((x,i)=>{let y=10+i*rh;text(s,6,y+11,x.target,'lbl','start');let w=Math.max(2,585*x.calls/max),color=`hsl(${120*x.success_rate} 75% 58%)`;s.append(el('rect',{x:6,y:y+16,width:w,height:12,rx:3,fill:color,opacity:.85}));text(s,12+w,y+26,pct(x.share)+' · '+pct(x.success_rate),'tick');hover(s.lastChild,`${route.route_id} → ${x.target}\n${fmt(x.calls,0)} calls (${pct(x.share)})\nSuccess ${pct(x.success_rate)}\nReward ${pct(x.mean_reward)}\np95 ${fmt(x.p95_latency_ms/1000,2)}s\nBuckets ${JSON.stringify(x.buckets)}`)});$('#selectorDesc').textContent=`${route.route_id}: ${fmt(route.answer_calls,0)} routed answers · ${fmt(route.classifier_calls,0)} logged classifier calls`;$('#selector').replaceChildren(s)}
...
async function load(){let id=$('#runs').value;if(!id)return;$('#main').classList.add('loading');let r=await fetch('/api/runs/'+encodeURIComponent(id)+'/summary');data=await r.json();render()}
async function init(){let runs=await (await fetch('/api/runs')).json(),s=$('#runs');s.innerHTML=runs.map(r=>`<option value="${r.id}">${new Date(r.created_at).toLocaleString()} · ${r.run_id||'run'}</option>`).join('');s.onchange=load;await load()}init().catch(e=>{$('#runMeta').textContent='Failed to load: '+e;$('#main').classList.remove('loading')});
</script></body></html>"""
